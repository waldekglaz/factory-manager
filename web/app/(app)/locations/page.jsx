...
export default function Locations() {
  ...
  async function submitForm(e) {
    ...
    try {
      const payload = {
        ...
        deliveryDays:
          form.isRemote && form.deliveryDays !== ""
  ...
  async function deleteLocation(loc, e) {
    ...
    if (total > 0) {
      setError(
        `Cannot delete "${loc.name}" — it still holds ${total} unit(s). Transfer all stock first.`,
      );
      return;
    }
  ...
  function openTransfer() {
    setTransferForm({
      itemId: "",
      fromLocationId: selected?.location.id ?? "",
      toLocationId: "",
      quantity: "",
    });
        ...
        <div>
          {locations.length === 0 ? (
            <div className="empty">
              ...
              <p>
                Create your first storage location to start tracking where
            ...
            >
              {locations.map((loc) => {
                ...
                const total =
                  (loc.totalPartUnits ?? 0) + (loc.totalProductUnits ?? 0);
                return (
                  <div
                    key={loc.id}
                    ...
                    >
                      <div>
                        ...
                        >
                          <span>
                            Parts:{" "}
                            <strong>{loc._count?.partStocks ?? 0}</strong> SKUs
                            / <strong>{loc.totalPartUnits ?? 0}</strong> units
                          </span>
                          <span>
                            Products:{" "}
                            <strong>{loc._count?.productStocks ?? 0}</strong>{" "}
                            SKUs / <strong>{loc.totalProductUnits ?? 0}</strong>{" "}
                            units
                          </span>
                        </div>
                      </div>
            ...
            <div>
                ...
                <div>
                  ...
                  {selected.location.isRemote && (
                    <span style={{ fontSize: 12, color: "#b45309" }}>
                      Remote location · {selected.location.deliveryDays ?? 0}{" "}
                      day delivery
                    </span>
                  )}
                </div>
              ...
              {selected.stock.partStocks?.length === 0 ? (
                <p className="text-muted" style={{ marginBottom: "1rem" }}>
                  No parts stock in this location.
                </p>
              ) : (
              ...
              {selected.stock.productStocks?.length === 0 ? (
                <p className="text-muted">
                  No finished goods in this location.
                </p>
              ) : (
        ...
        >
          ...
          >
            ...
            <form onSubmit={submitForm}>
              ...
              <div
                className="field"
                style={{ marginBottom: form.isRemote ? 12 : 16 }}
              >
                <label
                  style={{
                    display: "flex",
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...
                        deliveryDays: e.target.checked ? f.deliveryDays : "",
                      }))
                    }
                  />
                  Remote location{" "}
                  <span style={{ color: "#888", fontWeight: 400 }}>
                    (stock needs transit time before use)
                  </span>
                </label>
              </div>
              {form.isRemote && (
                <div className="field" style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
                    The planner will add this many days before stock from this
                    location is available for production.
                  </div>
                </div>
              )}
              <div className="gap-2">
              ...
              <div className="field" style={{ marginBottom: 12 }}>
                ...
                {transferType === "parts" ? (
                  <select
                    ...
                    onChange={(e) =>
                      setTransferForm((f) => ({ ...f, itemId: e.target.value }))
                    }
                    required
                    style={{ width: "100%" }}
                  >
